...
static int fcgi_demux_response(server *srv, handler_ctx *hctx) {
	 ...
	 *
	 * Ok, to be honest:
	 * - it is fine to receive an EAGAIN on a second read() call
	...
	case NETWORK_STATUS_WAIT_FOR_EVENT:
		/* an EAGAIN after we read exactly the chunk-size */

		ERROR("%s", "oops, got an EAGAIN even though we just got called for the event; weird");
		return -1;
...
TRIGGER_FUNC(mod_fastcgi_handle_trigger) {
	...
	for (i = 0; i < srv->conns->used; i++) {
		...
		/* the request has been waiting for an FCGI_STDOUT for 60 seconds */
